...
library(ggmap)
potatos <- read.csv("C:/Users/Usuario/OneDrive/Documents/UOC/master_ciencia_dades/M2.959_Visualitzacio_de_dades/PAC2/2022_accidents_gu_bcn.csv")
gpotato <- ggplot(potatos, aes(aes(xmin = Longitud, ymin = Latitud,
                                   xmax = Longitud + 0.5, ymax = Latitud + 0.5))) +
  geom_rect()


indice <- sample(1:nrow(potatos), 500, replace = FALSE)
subconjunto <- potatos[indice, ]
